...
- [x] 5.1 创建 `docs/` 目录（若不存在）
- [x] 5.2 创建 `docs/data_flow_spec.md`，内容包含：数据源目录（API 名、数据集、T+N 延迟、单位注意事项）、缓存文件清单（文件名、列名、更新频率）、数据流链路 Mermaid 图（Remote API → CSV → @st.cache_data → session_state → UI）、已知性能瓶颈（backfill 慢路径、串行 fetch、重复读）、优化建议（并行化、regime 结果磁盘持久化、消除重复 fetch）
